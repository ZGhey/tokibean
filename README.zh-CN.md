...
**Codex**(只有装了 Codex 才会显示):

1. 点 **「安装 Codex hooks」** → 写入 `~/.codex/hooks.json`(先备份)
2. **在 Codex 里执行 `/hooks` 批准这些 hook。** 这步不能省:Codex 会给每个 hook 定义算哈希,不经你批准就**拒绝执行**,而且任何改动都会重新触发审核。在你批准之前,面板会诚实地写「**待批准**」——hooks 写进去了,但还没生效。
3. 在 Codex 里随便跑点什么 → 面板翻成「**已生效**」,因为**真的收到了事件**
