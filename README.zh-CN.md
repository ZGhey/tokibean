...
Claude Code、Codex、Hermes 都盯。它陪你思考、写码、搜索、完工撒花——顺便帮你盯着额度。
...
内置六套角色——橘猫、柴犬、招财猫、达摩、兔儿爷…… **[看全员登场 ↓](#皮肤图鉴)**
...
住在桌面上的 AI 编程 agent 状态监视宠物——支持 **Claude Code**、**Codex** 和 **Hermes**。不用喂食、不用养成,它只做三件事:
...
2. **实时额度与 token 用量**:Claude 的 5 小时窗口和 Codex 自己的窗口并排各占一张卡——各有各的长度、各有各的重置时刻,**绝不混算**。Hermes 没有公开限额,给它一张纯 token 计数卡。外加今日/本周的全 agent token 总量。
3. **额度状态可视化**:**任一** agent 用量超 80%,头顶就冒感叹号;但只有**所有** agent 都耗尽才躺平睡觉(Codex 还有额度,就还有活可干)。

跨平台:Windows / macOS / Linux(Tauri 2)。

**和其他编程 agent 桌宠的区别:** Claude 这边它接的是 **Anthropic 官方用量 API**(面板里一次性 OAuth 连接账号),所以 5 小时窗口百分比和重置倒计时是真实数字,不是估算。另外它有一整套环境系统——真实月相、四季、天气、节日,全部离线由本地时钟算出——还有[六套内置皮肤](#皮肤图鉴)住在里面。

Claude Code 是旗舰:官方用量 API、应用内连接账号、WSL hook 同步都只有它有。Codex 是一等公民但更简单——它的额度白送在本地日志里,不用登录。Hermes 走进程内 Python 插件,完整支持**多 profile**:每个 profile 都装上、都统计,token 用量直接读 Hermes 本地数据库。
...
六套内置角色,面板下拉即时切换。同一颗脑子,不同的魂:每套皮肤都用自己的肢体语言重演全套状态——"改代码"在每只身上都认得出来,但没有两只演得一样。

| | | |
| :---: | :---: | :---: |
| ![拱门·墩墩](docs/gifs/skin-dundun.gif) | ![橘猫·摸鱼](docs/gifs/skin-tabby.gif) | ![柴犬·豆豆](docs/gifs/skin-shiba.gif) |
| **拱门·墩墩**(默认)<br>原创柿子橙拱门,全场行头最多:思考顶爱因斯坦头,写码戴工程帽,读文件换博士帽。 | **橘猫·摸鱼**<br>是真猫,不是披猫皮的程序员——搜索靠钻纸箱翻找,还会追激光笔、把杯子扒下桌。 | **柴犬·豆豆**<br>全程狗语:干活摇尾巴,等你时竖起耳朵,完工吐着舌头原地撒欢。 |

| | | |
| :---: | :---: | :---: |
| ![招财猫·墩墩](docs/gifs/skin-maneki.gif) | ![达摩·许愿](docs/gifs/skin-daruma.gif) | ![兔爷·骑虎](docs/gifs/skin-tuerye.gif) |
| **招财猫·墩墩**<br>招手速度就是心情——闲时慢悠悠地晃,干活双倍速"招财",庆祝直接三倍速。红坐垫从不离身。 | **达摩·许愿**<br>没有手脚,全凭意志——干活就是原地摇而不倒。完工那一刻,空着的右眼被一笔点上——愿望达成。 | **兔爷·骑虎**<br>兔儿爷端坐虎背,将军范儿从不破功;戏全在老虎身上——眨眼、甩尾、飞奔、被吓一跳,连眼珠都跟着你的光标转。 |

皮肤还能自动轮换:**设置 → 皮肤循环**,每小时或每天(对齐整点/本地午夜)在勾选的皮肤间轮换。当前皮肤由时钟推导,重启落在同一张;手动选中某张皮肤即自动关闭循环。想自己画一套?见[自制皮肤](#自制皮肤)。
...
1. 点 **「安装 hooks」** → 把 `tokibean` 插件写进**每个 profile** 自己的 `plugins/tokibean/` 目录,并在各 profile 的 `config.yaml` 里启用(写入前备份为 `config.yaml.bak-tokibean`)。标签页会列出找到的全部 profile——默认的 `~/.hermes` 加上每个 `~/.hermes/profiles/<name>/`
2. **重启 Hermes** 让它加载插件
3. 在 Hermes 里随便跑点什么 → 宠物有反应,会话列表还会标明来自哪个 profile
...
Hermes 插件       ──POST──▶ 127.0.0.1:8737/event/hermes?profile=<name>   ─┘

~/.claude/projects/*.jsonl  ──增量解析──▶ 5 小时窗口       ─┐
~/.codex/sessions/*.jsonl   ──增量解析──▶ Codex 的窗口      ├─▶ 各占一张额度卡
~/.hermes/**/state.db       ──SQLite 扫描──▶ Hermes token 数 ─┘
```
...
- **多 agent、多会话**:按 `(agent, profile, session_id)` 独立记状态——agent 身份在**安装时**由 hook 的 URL 路径决定(**绝不从 payload 里猜**),profile 走查询参数(只有 Hermes 有,Claude/Codex 为空)。多路并行时头顶显示 ×N 徽章(跨 agent 合计);任一路在干活就算干活,全部完工才庆祝。用量面板按 agent + 项目目录逐一列出每个会话的状态和时长。
...
- **Hermes 走插件,不是 shell hook**:Tokibean 装的是进程内 Python 插件,把 Hermes 的生命周期事件(工具调用、LLM 回合、审批请求、会话边界)转发给宠物。插件装进每个 profile,事件自带 profile 名,所以 `work` 和 `default` 两个 profile 的会话互不混淆。token 用量每 30 秒从各 profile 的 `state.db`(SQLite)汇总;Hermes 不公开限额,所以它的卡只显示 token 数、没有百分比——也永远不会让宠物躺平睡觉。
...
打开 `~/.claude/settings.json`(和/或 `~/.codex/hooks.json`),删掉 hooks 里所有 `command` 包含 `127.0.0.1:8737/event` 的条目即可;或直接用备份文件恢复——Claude Code 是 `settings.json.bak-tokibean`,Codex 是 `hooks.json.bak-tokibean`。Hermes:把各 profile `config.yaml` 里 `plugins.enabled` 中的 `tokibean` 删掉(备份:`config.yaml.bak-tokibean`),再删掉 `plugins/tokibean/` 目录。

## 自制皮肤

[六套内置皮肤](#皮肤图鉴)每套都是 `src/skins/` 下覆盖 `window.PetRenderer` 的独立文件,可复用 `window.PetKit` 工具箱(像素/气泡/状态框/爱心/彩纸)——你自己画的也一样。
...
- **Hermes 没有额度百分比**:Hermes 不公开限额数据,它的卡只显示 token 数——没有进度条、没有 80% 警告,也不会让宠物躺平睡觉。
