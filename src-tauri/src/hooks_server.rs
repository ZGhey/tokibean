...
pub fn run(app: AppHandle, shared: Arc<Shared>) {
    ...
    for mut request in server.incoming_requests() {
        ...
        handle_event(&app, &shared, &agent, &profile, &body);
    }
}
...
///
/// Returns (agent_slug, profile). Bare `/event` means claude, so every hook installed by an earlier
/// version keeps working with no reinstall. An unknown agent slug is treated as claude rather than
/// dropped, so a future typo in a hook config degrades instead of silently swallowing events.
///
/// Profile is extracted from the `?profile=` query parameter (Hermes bridge bakes it in). Empty
/// string when absent — Claude and Codex have no profile concept.
fn agent_from_path(url: &str) -> (String, String) {
    let agent = {
    ...
    let profile = url
        ...
        .and_then(|qs| {
            qs.split('&')
                .find(|p| p.starts_with("profile="))
                .map(|p| p.trim_start_matches("profile="))
        })
        .unwrap_or("");
...
mod tests {
    ...
    fn the_bare_event_path_still_means_claude() {
        ...
        assert_eq!(agent_from_path("/event"), ("claude".into(), "".into()));
        assert_eq!(agent_from_path("/event/"), ("claude".into(), "".into()));
    }

    #[test]
    fn the_path_names_the_agent() {
        assert_eq!(agent_from_path("/event/codex"), ("codex".into(), "".into()));
        assert_eq!(agent_from_path("/event/claude"), ("claude".into(), "".into()));
    }

    #[test]
    fn a_query_string_does_not_confuse_the_slug() {
        assert_eq!(agent_from_path("/event/codex?x=1"), ("codex".into(), "".into()));
    }
    ...
    fn an_unknown_agent_degrades_to_claude_rather_than_vanishing() {
        ...
        assert_eq!(agent_from_path("/event/gemini"), ("claude".into(), "".into()));
        assert_eq!(agent_from_path("/event/../etc"), ("claude".into(), "".into()));
    }

    #[test]
    fn hermes_profile_from_query_param() {
        assert_eq!(agent_from_path("/event/hermes?profile=work"), ("hermes".into(), "work".into()));
    }

    #[test]
    fn profile_empty_when_missing() {
        assert_eq!(agent_from_path("/event/hermes"), ("hermes".into(), "".into()));
    }
}

fn handle_event(app: &AppHandle, shared: &Shared, agent: &str, profile: &str, body: &str) {
    ...
    let session_key = SessionKey::new(agent, profile, v["session_id"].as_str().unwrap_or("default"));
